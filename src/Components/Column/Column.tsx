...
const Column: React.FC<{
  ...
  SetStateInLocal(state: string, key: string): void;
}> = (props) => {
  ...
  const HandlerNameColumn = (event) => {
    ...
    const arrayColumns = JSON.parse(localStorage.getItem('nameBoard') || '[]');
    arrayColumns[props.propsForColumn.id] = {
      nameColumns: event.target.value,
      id: props.propsForColumn.id,
    };
    console.log(arrayColumns);
    props.SetStateInLocal('nameBoard', JSON.stringify(arrayColumns));
  };
  return (
